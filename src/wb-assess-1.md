...
```
Console.log outputs the results to the terminal for viewing. Return saves the values/repsonses for later use. 
```

**_What's the difference between defining and calling a function?_**

```
Defining a function is the list of code instructions associated with the function. Calling the function refers to invoking the function, or executing the code. 
```
...
```
A string is a data type that is text. It cannot be aggregated or manipulated like numebrs. Text can be concatentated. 
```

**_Number_**

```
A number data type can be aggregated and manipulated with operators like addition, subtraction and others. 
```

**_Boolean_**

```
Boolean is a true or false statement. It can be referred to as on/off, 1/0, T/F, true/false, or True/False. 
```

**_Array_**

```
Array is a list of items.  These items can be indexed and used. More items can be added to it and items can be removed. The items can also be referenced. Many things can be done to this 'list' called an array. 
```
